...
import { getDailySchedule, getTomorrowDate } from "~/lib/data-loader";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const schedule = await getDailySchedule(tomorrow);
  
  return { schedule };
}

export default function ModelTomorrow() {
      ...
      <div className="container mx-auto px-4 py-4 sm:py-6">
        {schedule.matches.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-muted-foreground text-lg">No matches scheduled for tomorrow</p>
            <p className="text-muted-foreground mt-2">Check back later or view today's results</p>
          </div>
        ) : (
